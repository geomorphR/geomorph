...
#' can be used with \code{\link{plotRefToTarget}} to generate gpaphical represntations of shape change, based on prediction criteria. 
#' 
#' @param A An array (p x k x n) containing Procrustes residuals either from GPA or fitted values from a previous
...
#' @param method A choice betwen least squares (LS) or partial least squares (PLS) regression for prediction.  The function defaults
...
#' PLS analysis using  \code{\link{two.b.pls}} should be performed first, as only the first singular vector for pedictors will be used
...
#'
#' Function estimates one or more configurations based on one or more linear predictors, such as PC scores
#' allometric relationships, or any other least squares or partial least squares regression.  These configuraions
#' can be used with \code{\link{plotRfToTarget}} to generate gpaphical represntations of shape change of the 
#' man shape, based on prediction criteria. 
#' 
#' @param A An array (p x k x n) containing Procrustes residuals either from GPA or fitted values from a previous
#' analytical procedure. 
#' @param x Linear (numeric) predictors.  Can be a vector or a matrix, or a list containing vectors or matrices.  Values must
#' be numeric.  If a factor is desired, one should use \code{\link[stats]{model.matrix}} to obtain a design matrix.  This will
#' impact how prediction criteria need to be provided (see below).
#' @param Intercept Logical value to indicate if an intercept should be used in the linear equation for predictions.  Generally, 
#' this value will be FALSE for shape predictions made in ordination plots.  It should be TRUE in cases where the expected
#' shape at the point the predictor has a value of 0 is not the mean shape.
#' @param method A choice betwen least squares (LS) or partial least squares (PLS) regression for prediction.  The function defaults
#' to LS prediction.  PLS might be chosen in cases where correlation is preferred over linear regression.  If PLS is chosen, a two-block
...
#' a vector (if more than one predictor or a prediciton matrix is provided); e.g., pred1 = c(0.1, -0.5), pred2 = c(-0.2, -0.1) (which
#' would be the case if two predictors were provided).  It is essential that the number of elements in any prediction criterion matches
#' the number of predictors.  Caution should be used when inputting a design matrix to ensure that correct dummy variables are used in
#' prediction criteria, and that an intercept is not TRUE if alrady included in the design matrix.
#' @export
#' @author Michael Collyer
#' @return A list of predictd shapes matching the number of vectors of prediction criteria provides.  The predications 
#' also have names matching those of the prediction criteria.
#' @examples
#' # Examples using Plethodon data
#'
...
#'plotTangentSpace(Y.gpa$coords)
#'
#'preds <- shape.predictor(Y.gpa$coords, x= NULL, Intercept = F, pred1 = -0.1, pred2 = 0.1) # PC 1 extremes, sort of
#'M <- mshape(Y.gpa$coords)
#'plotRefToTarget(M, preds$pred1)
#'plotRefToTarget(M, preds[[1]]) # same result
#'plotRefToTarget(M, preds$pred2)
...
#'PCA <- plotTangentSpace(Y.gpa$coords)
#'PC <- PCA$pc.scores[,1]
#'preds <- shape.predictor(Y.gpa$coords, x= PC, Intercept = F, pred1 = min(PC), pred2 = max(PC)) # PC 1 extremes, more technically
#'plotRefToTarget(M, preds$pred1)
#'plotRefToTarget(M, preds$pred2)
#'
#'PC <- PCA$pc.scores[,1:2]
#'# user-picked spots - can be anything, but it in this case, apparent groups
#'preds <- shape.predictor(Y.gpa$coords, x= PC, Intercept = F, 
#'                         pred1 = c(0.045,-0.02), pred2 = c(-0.025,0.06), pred3 = c(-0.06,-0.04)) 
#'plotRefToTarget(M, preds$pred1)
#'plotRefToTarget(M, preds$pred2)
#'plotRefToTarget(M, preds$pred3)
...
#'plethAllometry <- procD.allometry(coords~Csize, data=gdf)
#'plot(plethAllometry, method="RegScore")
#'preds <- shape.predictor(plethAllometry$Ahat, x= plethAllometry$Reg.proj, Intercept = F, 
#'                         predmin = min(plethAllometry$Reg.proj), 
#'                         predmax = max(plethAllometry$Reg.proj)) 
#'plotRefToTarget(M, preds$predmin, mag=3)
#'plotRefToTarget(M, preds$predmax, mag=3)
#'
#'plot(plethAllometry, method="PredLine")
#'preds <- shape.predictor(plethAllometry$Ahat, x= plethAllometry$pred.val, Intercept = F, 
#'                         predmin = min(plethAllometry$pred.val), 
#'                         predmax = max(plethAllometry$pred.val)) 
#'plotRefToTarget(M, preds$predmin, mag=3)
#'plotRefToTarget(M, preds$predmax, mag=3)
...
#'preds <- shape.predictor(Y.gpa$coords, plethShapeFood$food, Intercept = FALSE,
#'                         method = "PLS",
#'                         pred1 = 4, pred2 = -2) # using PLS plot as a guide
#'plotRefToTarget(M, preds$pred1, mag=2)
#'plotRefToTarget(M, preds$pred2, mag=2)
#'
shape.predictor <- function(A, x = NULL, Intercept = FALSE, method = c("LS", "PLS"), ...){
...
>>>>>>> 312552636a32714fcacbdc0292f1641a59c45920
